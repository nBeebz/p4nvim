...
M.changes = {
  args = {
    { status = '-s %s', client = '-c %s', max = '-m %d', reverse = '-r', time = '-t', user = '-u %s', me = '--me', long = '-l' },
    { file = '%s', revision = '%s' },
  },
...
M.copy = {
  args = { { change = '-c %s', branch = '-b %s', preview = '-n', force = '-F', max = '-m %d' }, { from = '%s', to = '%s' } },
}
...
-- ... client
M.opened = {
  args = { { short = '-s', change = '-c %s', default = '-c default', server = '-a', client = '-C %s', max = '-m %d', user = '-u %s' }, { file = '%s' } },
  default = { short = true },
  list = true,
}

M.reconcile = {
  args = { clean = '-w', preview = '-n', modtime = '-m', change = '-c %s', added = '-a', deleted = '-d', edited = '-e' },
  default = { clean = true },
}
...
M.revert = {
  args = { { unchanged = '-a', change = '-c %s', keep = '-k', preview = '-n', wipe = '-w' }, { all = '//...', files = '%s' } },
  default = { all = true },
}
...
-- ... fileSize
M.sync = {
  args = { { force = '-f', preview = '-n', preview_network = '-N', reopen = '-r', safe = '-s', max = '-m %d' }, { file = '%s' } },
  canary = 'action',
}

M.unshelve = {
  args = { shelf = '-s %s', change = '-c %s', force = '-f', branch = '-b %s', preview = '-n', parent = '-P %s', stream = '-S %s' },
}
...
for cmd, t in pairs(M) do
  setmetatable(t, {
    __call = function(t, args, handler)
      if vim.fn.executable('p4') == 0 then return end
      args = args or {}
